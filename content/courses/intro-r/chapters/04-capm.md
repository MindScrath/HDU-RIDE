...
- $R_m$：市场组合收益率
- $\beta_i$：个股对市场波动的敏感度
- $\alpha_i$：超额收益（理论上应不显著异于零）

检验 CAPM 的核心就是跑一次线性回归，看看 $\alpha$ 是否显著、$\beta$ 是多少。
...
关注 `summary` 输出中的：
- **截距项（Intercept）**：即 $\alpha$，若 p 值大于 0.05，则 $\alpha$ 不显著，符合 CAPM 预期
- **ret_index 系数**：即 $\beta$，衡量个股的系统性风险
...
- 换一只股票、换一个时间段，$\beta$ 会不同，这就是 CAPM 的"稳定性"问题
- 如果 $\alpha$ 显著不为零，说明该股票存在超出 CAPM 解释范围的超额收益
